...
begin
  if (i < 0) or (i > Length(s)) then // Si la posición está fuera de los límites del string, devolvemos un string vacío
    CharAt := ''
  else // Si la posición es válida, devolvemos el carácter en la posición indicada
    CharAt := Copy(s, i+1, 1);
end;
...
begin
  if Length(s) = 0 then // Si el string está vacío, devolvemos un string vacío
    LastChar := ''
  else // Si el string tiene al menos un carácter, devolvemos el último carácter
...
var
  iPos: integer;
begin
  iPos := Pos(a, b); // Buscamos la posición de la subcadena a en la cadena b
  if iPos = 0 then // Si no se encuentra, devolvemos -1
    indexOf := -1
  else
    indexOf := iPos - 1; // Si se encuentra, devolvemos la posición encontrada
end;
...
begin
  lastPos := 0; // Inicializamos la última posición en 0
  repeat
    ipos := Pos(a, b); // Buscamos la posición de la subcadena a en la cadena b
    if ipos > 0 then // Si se encuentra, actualizamos la última posición
    begin
      lastPos := ipos - 1;
      b := Copy(b, ipos + 1, Length(b) - ipos); // Avanzamos la cadena b para buscar la próxima ocurrencia
    end;
  until ipos = 0; // Salimos del bucle cuando no se encuentran más ocurrencias
  if lastPos = 0 then // Si no se encontró ninguna ocurrencia, devolvemos -1
    lastIndexOf := -1
  else
    lastIndexOf := lastPos; // Si se encontró al menos una ocurrencia, devolvemos la última posición encontrada
end;
